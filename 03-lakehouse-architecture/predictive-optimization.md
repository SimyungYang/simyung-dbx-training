...
```
Phase 1: 워크로드 분석 (지속적)
  ├── 테이블별 쓰기 패턴 분석 (빈도, 크기, 시간대)
  ├── 쿼리 패턴 분석 (어떤 컬럼이 자주 필터링되는지)
  ├── 소형 파일 비율 모니터링
  ├── 데이터 스큐 감지
  └── 마지막 OPTIMIZE/VACUUM 이후 변경량 추적

Phase 2: 최적화 계획 수립
  ├── 비용-편익 분석: "지금 OPTIMIZE를 실행하면 향후 쿼리가 얼마나 빨라지는가?"
  ├── 우선순위 결정: 가장 효과가 큰 테이블부터
  ├── 실행 시간 선택: 워크로드가 낮은 시간대
  └── 리소스 할당: 서버리스 컴퓨트 요청

Phase 3: 자동 실행
  ├── OPTIMIZE: 소형 파일 병합, Deletion Vector 물리 반영
  ├── VACUUM: 보존 기간 지난 파일 삭제
  ├── ANALYZE: 컬럼 통계 갱신
  └── 결과 기록: 시스템 테이블에 실행 이력 저장
```
