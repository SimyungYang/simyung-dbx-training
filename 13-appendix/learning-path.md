...
```mermaid
graph TB
    PRE["00. 선행 지식<br/>(RDB, 빅데이터 역사)"]
    DF["01. 데이터 기초<br/>(DW vs DL, ETL/ELT)"]
    OV["02. Databricks 개요<br/>(플랫폼, Workspace)"]
    LH["03. 레이크하우스<br/>(Delta Lake, Medallion)"]
    CP["04. 컴퓨트<br/>(Spark, Cluster, SQL WH)"]
    DE["05. 데이터 엔지니어링<br/>(Auto Loader, SDP, Jobs)"]
    DW["06. 데이터 웨어하우징<br/>(DBSQL, AI 함수)"]
    UC["07. Unity Catalog<br/>(거버넌스, 권한, 리니지)"]
    BI["08. AI/BI<br/>(Lakeview, Genie)"]
    ML["09. 머신러닝<br/>(MLflow, Serving, Feature)"]
    AG["10. 에이전트 개발<br/>(RAG, Vector Search)"]
    LB["11. Lakebase<br/>(관리형 PostgreSQL)"]
    SC["12. 보안/거버넌스<br/>(네트워크, 시스템 테이블)"]
    AP["13. 부록<br/>(CLI, Asset Bundles)"]
...
graph LR
    ...
    B --> C["04 컴퓨트"]
    C --> D["09 머신러닝<br/>⭐ 핵심"]
    D --> E["10 에이전트 개발<br/>⭐ 핵심"]

    style D fill:#fce4ec,stroke:#c62828,stroke-width:3px
    style E fill:#fce4ec,stroke:#c62828,stroke-width:3px
```
