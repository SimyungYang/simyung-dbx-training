...
```mermaid
graph TD
    A[📥 ingest_orders<br/>Pipeline Task] --> C[🔄 transform_orders<br/>Notebook Task]
    B[📥 ingest_products<br/>Pipeline Task] --> C
    C --> D{✅ validate_data<br/>SQL Task}
    D -->|성공| E[📊 build_gold_tables<br/>dbt Task]
    D -->|실패| F[🚨 send_alert<br/>Notebook Task]
    E --> G[📈 update_dashboard<br/>SQL Task]
    E --> H[📧 notify_success<br/>Notebook Task]
...
flowchart TD
    A[워크로드 분석] --> B{데이터 크기?}
    B -->|< 10GB| C[2 Workers<br/>m5.xlarge]
    B -->|10~100GB| D[4~8 Workers<br/>r5.xlarge]
    B -->|100GB~1TB| E[8~16 Workers<br/>r5.2xlarge]
    B -->|> 1TB| F[Auto-scaling<br/>16~64 Workers]
