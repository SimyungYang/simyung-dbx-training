...
```mermaid
flowchart LR
    A["소스 데이터<br/>(Raw)"] --> B{"Expectations<br/>품질 검증"}
    B -->|"✅ 통과"| C["정상 데이터<br/>→ 다음 단계"]
    B -->|"⚠️ WARN"| D["경고 기록 +<br/>데이터 포함"]
    B -->|"🚫 DROP"| E["위반 행 제거<br/>(격리 가능)"]
    B -->|"❌ FAIL"| F["파이프라인 중지"]
...
```mermaid
flowchart TD
    subgraph WARN ["ON VIOLATION WARN"]
        W1["레코드 수신"] --> W2{"조건 검사"}
        W2 -->|통과| W3["테이블에 저장"]
        W2 -->|위반| W4["테이블에 저장 +<br/>위반 메트릭 기록"]
    end

    subgraph DROP ["ON VIOLATION DROP ROW"]
        D1["레코드 수신"] --> D2{"조건 검사"}
        D2 -->|통과| D3["테이블에 저장"]
        D2 -->|위반| D4["레코드 제거 +<br/>위반 메트릭 기록"]
    end

    subgraph FAIL ["ON VIOLATION FAIL UPDATE"]
        F1["레코드 수신"] --> F2{"조건 검사"}
        F2 -->|통과| F3["테이블에 저장"]
        F2 -->|위반| F4["파이프라인 즉시 중지<br/>+ 오류 보고"]
    end
```
...
```mermaid
flowchart LR
    A["Bronze<br/>(원본 데이터)"] --> B{"Expectations<br/>(WARN)"}
    B --> C["Silver<br/>(정상 데이터)"]
    B --> D["Quarantine<br/>(위반 데이터)"]
    D --> E["수동 검토 /<br/>자동 수정"]
    E -->|수정 완료| C
