...
```mermaid
graph TB
    subgraph L3["3️⃣ 비즈니스 메트릭"]
        B1["전환율 변화"]
        B2["매출 영향"]
        B3["사용자 만족도"]
    end
    subgraph L2["2️⃣ 모델 성능"]
        M1["예측 정확도"]
        M2["데이터 드리프트"]
        M3["피처 분포 변화"]
    end
    subgraph L1["1️⃣ 인프라 / 서빙"]
        I1["QPS / 지연시간"]
        I2["에러율"]
        I3["GPU/CPU 사용률"]
        I4["비용 (DBU/토큰)"]
...
```mermaid
graph LR
    REQ["사용자 요청"] --> AGENT["에이전트"]
    AGENT --> RETRIEVER["문서 검색<br/>(Vector Search)"]
    AGENT --> LLM["LLM 호출<br/>(Foundation Model)"]
    AGENT --> TOOL["도구 실행<br/>(SQL, API 등)"]
    AGENT --> RES["최종 응답"]
