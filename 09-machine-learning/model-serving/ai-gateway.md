...
## 왜 클라이언트 측 구현(LiteLLM 등)이 아닌 AI Gateway인가?

LLM 호출을 관리하기 위해 LiteLLM, LangChain Router 등 **클라이언트 측 라이브러리** 를 사용하는 팀이 많습니다. 그러나 프로덕션 환경에서는 이 접근 방식에 심각한 한계가 있습니다.

### 클라이언트 측 구현의 위험

| 문제 | 설명 |
|------|------|
| **API 키 분산** | 각 애플리케이션이 OpenAI/Anthropic API 키를 직접 보유합니다. 키가 유출되면 무제한 과금이 발생할 수 있습니다 |
| **거버넌스 부재** | 누가, 언제, 어떤 모델을 호출했는지 중앙에서 추적할 수 없습니다. 규제 감사에 대응하기 어렵습니다 |
| **비용 통제 불가** | 팀별 사용량 제한이 없어, 한 애플리케이션의 폭주가 전체 예산을 소진할 수 있습니다 |
| **가드레일 우회** | 클라이언트 측 필터는 개발자가 코드를 수정하면 쉽게 우회할 수 있습니다 |
| **버전 파편화** | 각 팀이 다른 버전의 라이브러리를 사용하여 동작이 일관되지 않습니다 |
| **모니터링 사각지대** | 로깅이 각 클라이언트에 분산되어 있어 전체 사용 패턴을 파악할 수 없습니다 |

### AI Gateway vs 클라이언트 측 라이브러리 비교

| 항목 | 클라이언트 측 (LiteLLM 등) | Databricks AI Gateway |
|------|--------------------------|----------------------|
| **배포 위치** | 각 애플리케이션 코드 내부 | Databricks 플랫폼 (중앙 관리) |
| **API 키 관리** | 각 앱이 직접 보유 (분산) | Databricks가 중앙 관리 (앱은 키를 모름) |
| **가드레일** | 코드 레벨 (우회 가능) | 플랫폼 레벨 (우회 불가) |
| **비용 제어** | 자체 구현 필요 | Rate Limiting 내장 |
| **모니터링** | 자체 로깅 구축 필요 | 추론 테이블 + 시스템 테이블 자동 기록 |
| **모델 전환** | 코드 수정 + 재배포 필요 | 엔드포인트 설정 변경만으로 전환 |
| **Unity Catalog 통합** | ❌ | ✅ 권한, 감사, 리니지 |
| **운영 부담** | 높음 (직접 유지보수) | 없음 (관리형 서비스) |

> 💡 **핵심**: AI Gateway는 **서버 측(Server-side)** 에서 동작하므로, 애플리케이션 코드를 변경하지 않아도 정책이 즉시 적용됩니다. 클라이언트 측 라이브러리는 각 앱을 하나하나 수정하고 재배포해야 합니다.
