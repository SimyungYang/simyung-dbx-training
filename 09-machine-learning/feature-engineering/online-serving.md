...
graph LR
    subgraph OFFLINE["오프라인 (학습/배치)"]
        FT["Feature Table<br/>(Delta Lake)"]
        TRAIN["모델 학습"]
        FT -->|"대량 스캔"| TRAIN
    end
    subgraph ONLINE["온라인 (실시간 서빙)"]
        OT["Online Table<br/>(밀리초 조회)"]
        EP["Model Serving<br/>Endpoint"]
        OT -->|"단건 조회"| EP
    end
    FT -->|"자동 동기화"| OT
...
```mermaid
graph LR
    SRC["Feature Table<br/>(Delta Lake)"] -->|"CDF 스트리밍"| SYNC["동기화<br/>파이프라인"]
    SYNC -->|"증분 업데이트"| OT["Online Table<br/>(키-값 스토어)"]
    APP["애플리케이션"] -->|"customer_id=12345"| EP["Model Serving<br/>Endpoint"]
    EP -->|"자동 피처 조회"| OT
    EP -->|"예측 결과"| APP
...
sequenceDiagram
    ...
    participant EP as 서빙 엔드포인트
    participant OT1 as Online Table<br/>(고객 피처)
    participant OT2 as Online Table<br/>(가맹점 피처)
    participant Model as ML 모델

    Client->>EP: customer_id, merchant_id, amount
    EP->>OT1: customer_id로 조회
    OT1-->>EP: avg_amount, count_30d, age
    EP->>OT2: merchant_id로 조회
    OT2-->>EP: fraud_rate, category
    EP->>Model: 전체 피처 조합 전달
    Model-->>EP: prediction: 0.87
    EP-->>Client: {"is_fraud": 0.87}
```
