...
```mermaid
graph LR
    STD["Standard Runtime<br/>Spark + Delta Lake"]
    ML["ML Runtime<br/>+ ML 라이브러리"]
    GPU["ML Runtime (GPU)<br/>+ CUDA/cuDNN"]
...
```mermaid
graph LR
    DATA["📊 데이터셋<br/>(Delta 테이블)"]
    PREP["🔧 자동 전처리<br/>(결측값, 인코딩)"]
    TRAIN["🏋️ 다중 모델 학습<br/>(XGBoost, LightGBM,<br/>RF, LR ...)"]
    EVAL["📈 자동 평가<br/>(교차 검증)"]
    BEST["🏆 최적 모델<br/>+ MLflow 로깅"]

    DATA --> PREP --> TRAIN --> EVAL --> BEST
```
...
graph TD
    Q1{"워크로드<br/>유형은?"}
    Q1 -->|"데이터 엔지니어링/SQL"| STD["✅ Standard Runtime"]
    Q1 -->|"ML 모델 학습"| Q2{"GPU가<br/>필요한가요?"}
    Q2 -->|"아니요<br/>(sklearn, XGBoost)"| MLR["✅ ML Runtime (CPU)"]
    Q2 -->|"예<br/>(PyTorch, LLM)"| Q3{"모델 크기는?"}
    Q3 -->|"소~중형 모델"| GPU_S["✅ ML Runtime GPU<br/>(g5.xlarge)"]
    Q3 -->|"대형 LLM 파인튜닝"| GPU_L["✅ ML Runtime GPU<br/>(p4d.24xlarge)"]

    style STD fill:#e3f2fd
    style MLR fill:#e8f5e9
    style GPU_S fill:#fff3e0
    style GPU_L fill:#ffebee
```
