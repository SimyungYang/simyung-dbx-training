...
프로덕션 모델 학습/서빙: LTS Runtime만 사용
├── 새 LTS 출시 → 테스트 환경에서 2주간 검증
├── 기존 모델 재현성 확인 (AUC, F1 등 메트릭 비교)
├── 검증 통과 → 프로덕션 전환
└── 검증 실패 → 이전 LTS 유지, 원인 분석

실험/PoC: 최신 Runtime 사용 가능
├── 새 라이브러리/기능 테스트
└── 모델 확정 후 LTS로 전환
```
...
실전 수치:
├── 1B 모델:  2GB (가중치) + 4GB (오버헤드) ≈ 6GB  → T4 (16GB) OK
├── 7B 모델:  14GB + 14GB ≈ 28GB → A10G (24GB) × 2 또는 A100 (40GB) × 1
├── 13B 모델: 26GB + 26GB ≈ 52GB → A100 (80GB) × 1 또는 A10G × 4
├── 70B 모델: 140GB + 140GB ≈ 280GB → A100 (80GB) × 4 이상
└── 405B 모델: 810GB+ → A100 × 16 이상 (멀티 노드 필수)
```
