...
-- 그룹 기반 권한 부여 패턴
-- 1. 카탈로그 접근 허용
GRANT USE CATALOG ON CATALOG prod_sales TO `data-analysts`;

-- 2. 스키마 접근 허용
GRANT USE SCHEMA ON SCHEMA prod_sales.gold TO `data-analysts`;

-- 3. 테이블 읽기 허용
GRANT SELECT ON SCHEMA prod_sales.gold TO `data-analysts`;

-- 4. 프로덕션 쓰기는 Service Principal만
GRANT MODIFY ON SCHEMA prod_sales.silver TO `svc-prod-pipeline`;
GRANT MODIFY ON SCHEMA prod_sales.gold TO `svc-prod-pipeline`;

-- 5. 소유권 이전 (개인 → 그룹)
ALTER TABLE prod_sales.gold.daily_revenue
SET OWNER TO `data-engineers`;
```
...
├──────────────────────────────────────────┤
│                                          │
│  1. 기본: 모든 권한 거부 (Deny by Default) │
│     ↓                                    │
│  2. USE CATALOG 부여 (접근 허용)           │
│     ↓                                    │
│  3. USE SCHEMA 부여 (스키마 탐색)          │
│     ↓                                    │
│  4. SELECT 부여 (읽기)                    │
│     ↓                                    │
│  5. 필요한 경우에만 MODIFY 부여 (쓰기)      │
│     ↓                                    │
│  6. CREATE TABLE은 팀 스키마에만 부여       │
│                                          │
└──────────────────────────────────────────┘
```
