...
graph LR
    ...
    SQL --> EVAL{"조건 평가<br/>value > threshold?"}
    EVAL -->|"Yes"| STATE_CHG{"상태 변경?"}
    EVAL -->|"No"| OK["🟢 OK 상태"]
    STATE_CHG -->|"OK → TRIGGERED"| ALERT["🔔 알림 발송"]
    STATE_CHG -->|"이미 TRIGGERED"| NOTIFY{"알림 빈도<br/>설정 확인"}
    NOTIFY -->|"매번 알림"| ALERT
    NOTIFY -->|"한번만 알림"| SKIP["⏭️ 추가 알림 없음"]
    OK --> SCH
    ALERT --> SCH
    SKIP --> SCH
```
...
```mermaid
graph TD
    SCHED["⏰ 스케줄 트리거<br/>(매일 오전 9시)"]
    SCHED --> REFRESH["🔄 대시보드 데이터 갱신"]
    REFRESH --> CHECK{"구독자<br/>있는가?"}
    CHECK -->|"Yes"| SEND["📧 이메일 전송<br/>(PDF/링크/이미지)"]
    CHECK -->|"No"| DONE["✅ 갱신 완료"]
    SEND --> DONE
```
