...
```mermaid
graph TB
    subgraph AIBI["Databricks AI/BI"]
        DASH["📊 AI/BI Dashboard<br/>(Lakeview)"]
        GENIE["🧞 Genie"]
        ALERTS["🔔 Alerts"]
    end

    DATA["💾 Delta Lake<br/>(레이크하우스 데이터)"]
    WH["🖥️ SQL Warehouse<br/>(쿼리 실행)"]

    DATA --> WH --> AIBI

    DASH -->|"시각화"| U1["📈 차트, 표,<br/>KPI 카운터"]
    GENIE -->|"자연어 → SQL"| U2["💬 질문하면<br/>바로 답변"]
    ALERTS -->|"조건 모니터링"| U3["📧 이메일/Slack<br/>알림 발송"]
```
...
graph LR
    subgraph DataLayer["데이터 레이어"]
        B["🥉 Bronze"] --> S["🥈 Silver"] --> G["🥇 Gold<br/>(분석용 테이블)"]
    end

    subgraph BILayer["BI 레이어"]
        WH["SQL Warehouse"]
    ...
    subgraph External["외부 BI 도구"]
        TAB["Tableau"]
        PBI["Power BI"]
        LOOK["Looker"]
    end

    WH -->|"JDBC/ODBC"| External
```
