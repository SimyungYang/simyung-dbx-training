...
```mermaid
graph TB
    subgraph "Databricks 책임"
        D1["🏢 Control Plane 인프라 보안"]
        D2["🔄 플랫폼 업데이트 / 보안 패치"]
        D3["🔒 기본 암호화 (TLS + AES-256)"]
        D4["📋 인증 유지 (SOC 2, ISO 27001)"]
    end

    subgraph "고객 책임"
        C1["🌐 네트워크 설정 (VPC, Private Link)"]
        C2["👥 사용자/그룹 관리"]
        C3["🛡️ 데이터 접근 정책 (GRANT/REVOKE)"]
        C4["🔑 CMK, IP Access List 설정"]
    end
