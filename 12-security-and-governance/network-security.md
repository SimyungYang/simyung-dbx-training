...
```
[상황]
- 금융사 데이터 분석팀이 Databricks 도입 (PoC → 프로덕션 전환)
- PoC 단계에서 네트워크 설정 없이 빠르게 구축
- 6개월 후 프로덕션 환경으로 전환, 고객 금융 데이터 적재
- 연례 보안 감사에서 다음 지적 사항 발견

[감사 지적 사항]
├── 1. Databricks 워크스페이스가 퍼블릭 인터넷에서 접근 가능
│   → "누구나 URL만 알면 로그인 페이지에 접근 가능합니다"
│   → 금감원 전자금융감독규정 위반 가능성
│
├── 2. Data Plane의 아웃바운드 트래픽이 제한되지 않음
│   → 클러스터에서 임의의 외부 IP로 데이터 전송 가능
│   → 데이터 유출 경로가 열려 있음
│
├── 3. VPC Flow Logs가 비활성화 상태
│   → 네트워크 트래픽 감사 추적 불가
│
└── 4. S3 버킷에 VPC 제한이 없음
    → Databricks 외부에서도 S3 데이터에 접근 가능

[결과]
├── 보안 감사 "부적합" 판정
├── 3개월 내 시정 조치 명령
├── 시정 비용: Private Link + VPC 재구성에 약 2억 원 (컨설팅 포함)
└── 프로덕션 워크로드 일시 중단 (2주)
```
