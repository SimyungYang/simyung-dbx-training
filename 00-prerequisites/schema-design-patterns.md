...
| 유형 | 테이블 | 주요 컬럼 |
|------|--------|----------|
| **팩트 테이블** | fact_orders | order_id, customer_id, product_id, date_id, store_id, quantity, amount |
| **디멘전 테이블** | dim_customer | customer_id, name, email, city, tier |
|  | dim_product | product_id, name, category, brand, price |
|  | dim_date | date_id, date, month, quarter, year, day_of_week |
|  | dim_store | store_id, name, address, region |

> Star 스키마에서는 중앙의 팩트 테이블이 주변 디멘전 테이블들과 직접 연결됩니다.
...
| 유형 | 테이블 | 상위 테이블 | 설명 |
|------|--------|-----------|------|
| **팩트 테이블** | fact_orders | - | 중앙의 팩트 테이블입니다 |
| **디멘전 테이블** | dim_customer | fact_orders | 고객 디멘전 |
|  | dim_product | fact_orders | 상품 디멘전 |
|  | dim_date | fact_orders | 날짜 디멘전 |
| **정규화된 서브 디멘전** | dim_category | dim_product | 상품 카테고리 |
|  | dim_brand | dim_product | 브랜드 |
|  | dim_city | dim_customer | 도시 |
|  | dim_region | dim_city | 지역 |

> Snowflake 스키마에서는 디멘전 테이블이 추가로 정규화되어 서브 디멘전 테이블로 분리됩니다.
...
| 계층 | 설명 | 스키마 특징 |
|------|------|-----------|
| **Bronze** | 원본 그대로 | 소스 시스템 구조 유지 |
| **Silver** | 정규화된 엔티티 | 정규화, 클렌징, 표준화 |
| **Gold** | Star 스키마 / 비정규화 | 분석에 최적화된 구조 |
