...
```mermaid
graph TB
    U["👤 '주문 12345 상태를 알려주세요'"] --> LLM["🧠 LLM 판단<br/>'get_order_status를<br/>호출해야겠다'"]
    LLM -->|"Tool Call"| TOOL["🔧 get_order_status(12345)"]
    TOOL -->|"결과: 배송중"| LLM2["🧠 LLM<br/>'결과를 자연어로<br/>답변하자'"]
    LLM2 --> A["💬 '주문 12345는 현재<br/>배송 중입니다.'"]
```
