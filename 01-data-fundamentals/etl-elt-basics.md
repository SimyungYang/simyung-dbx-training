...
| 단계 | ETL 프로세스 |
|------|-------------|
| 1. Extract (추출) | 소스 시스템 (DB, API, 파일) → ETL 서버 |
| 2. Transform (변환) | ETL 서버에서 정제, 표준화 |
| 3. Load (적재) | 변환된 데이터 → 데이터 웨어하우스 |
...
| 단계 | ELT 프로세스 |
|------|-------------|
| 1. Extract (추출) | 소스 시스템 (DB, API, 파일) → 데이터 레이크/레이크하우스 |
| 2. Load (적재) | 원본 그대로 저장 |
| 3. Transform (변환) | 레이크하우스 내에서 변환 (Spark, SQL) |
...
| 단계 | Databricks ELT (Medallion) |
|------|----------------------------|
| Extract & Load | 소스 → Bronze (원본 그대로 적재) |
| Transform | Bronze → Silver (정제, 표준화) |
| Transform | Silver → Gold (비즈니스 집계) |
| Serve | Gold → BI 대시보드, ML 모델 |
