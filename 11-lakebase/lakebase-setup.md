...
```mermaid
graph LR
    subgraph VPC_APP["앱 VPC"]
        APP["🌐 앱 서버"]
    end

    subgraph PRIVATE["Private Link"]
        APP -->|"프라이빗 연결<br/>(인터넷 미경유)"| ENDPOINT["Private<br/>Endpoint"]
    end

    subgraph VPC_DB["Databricks VPC"]
        ENDPOINT --> LB["Lakebase"]
    end
```
